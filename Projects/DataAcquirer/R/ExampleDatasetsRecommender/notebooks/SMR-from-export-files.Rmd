...
aWLDescriptions <- jsonlite::read_json( path = file.path( params$smrDirName, "WLExampleData-aDescriptions.json") )
aWLDescriptions <- setNames(as.character(aWLDescriptions), names(aWLDescriptions))
```

```{r}
summary(as.data.frame(unclass(dfWLSMRMatrix), stringsAsFactors = T))
```

```{r}
summary(as.data.frame(unclass(dfWLExampleData), stringsAsFactors = T))
```
...
aRDescriptions <- jsonlite::read_json( path = file.path( params$smrDirName, "RExampleData-aDescriptions.json") )
aRDescriptions <- setNames(as.character(aRDescriptions), names(aRDescriptions))
```

```{r}
summary(as.data.frame(unclass(dfRSMRMatrix), stringsAsFactors = T))
```

```{r}
summary(as.data.frame(unclass(dfRExampleData), stringsAsFactors = T))
```
...
```{r}
if( params$exportQ ) {
  smrWLDatasets %>% SMRExportToDirectory( directoryPath = "../output/", dataNamePrefix = "WLExampleData", format = "CSV")
}
```
