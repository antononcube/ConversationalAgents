(*
    Data acquisition dialogs finite state machine interface Mathematica code
...
(* :Discussion:
    ...
    This dynamic interface utilized the functions from the file:
     ...
     https://github.com/antononcube/ConversationalAgents/blob/master/Projects/DataAcquistionDialogsAgent/Mathematica/DataAcqusitionFSM.m
...
Magnify[
  DynamicModule[{input = "", fsmState = "WaitingForARequest",
    ...
    ColumnForm[{
      Panel[
        Row[
          {Style["speech input : ", Blue],
            ...
            Button["Find again...", (fsmState = "WaitingForARequest"; fsmMessage = {};
            speechMessage = ""; visualMessage = ""; input = "";
            fsmContext = DeleteCases[fsmContext, "filters" -> _]),
              Enabled -> Dynamic[If[fsmState == "LoadDataset", True, False]],
              ImageSize -> Large], " ",
            Button["Load dataset",
      ...
      Dynamic[
        If[fsmState != "LoadDataset" && StringLength[input] > 0,
          {fsmState, fsmMessage, fsmContext, speechMessage, visualMessage} = PCESM[fsmState, fsmContext, input];
          If[fsmState == "ListOfDatasetsForAQuery" || fsmState == "PrioritizedList",
